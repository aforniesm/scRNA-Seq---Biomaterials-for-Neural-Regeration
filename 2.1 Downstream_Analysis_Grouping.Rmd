...
pattern <- "nervous system development|nervous system process|regulation of apoptotic signaling pathway|neuron|neurogenesis|axon|axogenesis|spinal|synapse|acute|innate|plasticity|regulation of autophagy|regulation of inflammatory response"
...
# Filter the GO terms from the enrichGO object directly
pattern <- "nervous system development|nervous system process|regulation of apoptotic signaling pathway|neuron|neurogenesis|axon|axogenesis|spinal|synapse|acute|innate|plasticity|regulation of autophagy|regulation of inflammatory response|Wnt|Notch|phosphatidylinositol|MAPK|NF-kappaB|BMP|STAT|SMAD|toll-like|Rho"
filtered_go_res <- go_res
filtered_go_res@result <- filtered_go_res@result %>%
  dplyr::filter(stringr::str_detect(Description, regex(pattern, ignore_case = TRUE)))
...
go_categories <- list(
  `Neuronal development and specification` = c(
    "neurogenesis", "neuron differentiation", "fate specification", "fate commitment",
    "maturation", "neuron migration", "cell-cell adhesion", "neuron remodeling"
  ),
  `Axonal growth and regeneration` = c(
    "axon", "axonogenesis", "guidance", "regeneration", "projection extension",
    "projection regeneration", "ensheathment", "arborization"
  ),
  `Synapse development and plasticity` = c(
    "synapse", "synaptic", "pruning", "assembly", "organization", "plasticity", "postsynapse"
  ),
  `Spinal cord development` = c(
    "spinal cord", "ventral spinal cord", "motor neuron", "cell differentiation in spinal cord"
  ),
  `Nervous system patterning and regulation` = c(
    "nervous system development", "nervous system process",
    "regulation of nervous system", "neuron projection development"
  ),
  `Immune and inflammatory processes` = c(
    "immune", "inflammatory", "acute-phase", "antigenic", "immunological",
    "innate immune", "immunological synapse"
  ),
  `Apoptosis and neuronal survival` = c(
    "apoptotic", "cell survival"
  ),
  `Autophagy and cellular stress responses` = c(
    "autophagy", "stress"
  ),
  `TGF-B Signaling` = c(
...
col_fun <- colorRamp2(
  c(0, 1, 3, 6, 9),
  c("#ebebf3", "#d1d3ea", "#b0b3d9", "#888cc2", "#666ca8"))
...
hm_binary_non_signaling <- Heatmap(matrix_binary_non_signaling,
                               ...
                               heatmap_legend_param = list(title = "GO Terms per Category", at = c(0, 9),
                                                           labels = c("0", "9")),
                               column_title = " ",
                               column_title_gp = gpar(fontsize = 12),
                               bottom_annotation = column_annotation)
...
# Non-signaling
png("~/R/Figures2/Enrichment_NonSignaling_7dpi_Grouped.png", width = 5000, height = 2500, res = 500)
draw(hm_binary_non_signaling + hm_count_non_signaling + hm_pval_non_signaling, annotation_legend_list = list(regulation_legend), annotation_legend_side = "right")
dev.off()
...
# Filter the GO terms from the enrichGO object directly
pattern <- "nervous system|nervous system process|regulation of apoptotic signaling pathway|axogenesis|neuron|neurogenesis|axon|spinal|synapse|chronic|long-term|plasticity|regulation of autophagy|regulation of inflammatory response"
...
# Filter the GO terms from the enrichGO object directly
pattern <- "nervous system|nervous system process|regulation of apoptotic signaling pathway|axogenesis|neuron|neurogenesis|axon|spinal|synapse|chronic|long-term|plasticity|regulation of autophagy|regulation of inflammatory response|Wnt|Notch|phosphatidylinositol|MAPK|NF-kappaB|BMP|STAT|SMAD|toll-like|Rho"
filtered_go_res <- go_res
filtered_go_res@result <- filtered_go_res@result %>%
  dplyr::filter(stringr::str_detect(Description, regex(pattern, ignore_case = TRUE)))
...
gene_go <- gene_go[!grepl("glutamatergic", gene_go$go_term), ]
gene_go <- gene_go[!grepl("excitatory", gene_go$go_term), ]
gene_go <- gene_go[!grepl("homotypic", gene_go$go_term), ]
gene_go <- gene_go[!grepl("cell-matrix", gene_go$go_term), ]
gene_go <- gene_go[!grepl("neuron projection extension involved in neuron projection guidance", gene_go$go_term), ]
gene_go <- gene_go[!grepl("protein localization to synapse", gene_go$go_term), ]
...
hm_binary_non_signaling <- Heatmap(matrix_binary_non_signaling,
                               ...
                               column_names_rot = 90,
                               column_names_gp = gpar(fontsize = 7),
                               heatmap_legend_param = list(title = "GO Terms Per Category", at = c(0, 12),
                                                           labels = c("0", "12")),
                               column_title = " ",
                               column_title_gp = gpar(fontsize = 12),
                               bottom_annotation = column_annotation)
