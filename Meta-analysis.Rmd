...
umap_meta <- DimPlot(combined_seurat, group.by = "dataset") + theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 7)) + labs(title = "UMAP Plot Before Integration", x = "UMAP 1", y = "UMAP 2")

ggsave("~/R/Figures/UMAP_BeforeIntegration_MetaAnalysis.png", umap_meta, dpi = 1080, height = 14, width = 8)
...
ggsave("~/R/Figures/Cells_MetaAnalysis.png", celltype, dpi = 1080, height = 7, width = 10)
...
ggsave("~/R/Figures/UMAP_AfterIntegration_MetaAnalysis.png", umap_dataset, dpi = 1080, height = 7, width = 12)
```
...
seurat_integrated@meta.data <- seurat_integrated@meta.data %>%
  mutate(celltype_grouped = case_when(
    celltype %in% c("Mature oligodendrocytes (MOL)", 
                    "Myelin forming oligodendrocytes (MFOL)", 
                    "Newly formed oligodendrocytes (NFOL)") ~ "Oligodendrocytes",
    ...
    celltype %in% c("B cells", "NK, T cells", "Myeloid, dividing", "Peripheral immune cells") ~ "Immune cells",
    celltype %in% c("Vascular cells", "Vascular endothelial cells") ~ "Vascular",
    celltype %in% c("Dorsal", "Ventral") ~ "Neurons",
    TRUE ~ "Other"
  ))
...
DEG <- FindMarkers(seurat_integrated_ME,  ident.1 = "7dpi", ident.2 = "Uninjured", logfc.threshold = 0, min.pct = 0.05, test.use = "MAST", layer = "data")
...
DEG2 <- FindMarkers(seurat_integrated_ME,  ident.1 = "1dpi", ident.2 = "Uninjured", logfc.threshold = 0, min.pct = 0.05, test.use = "MAST", layer = "data")
...
DEG3 <- FindMarkers(seurat_integrated_ME,  ident.1 = "4-6 wpi", ident.2 = "Uninjured", logfc.threshold = 0, min.pct = 0.05, test.use = "MAST", layer = "data")
...
DEG4 <- FindMarkers(seurat_integrated_ME,  ident.1 = "21 dpi", ident.2 = "Uninjured", logfc.threshold = 0, min.pct = 0.05, test.use = "MAST", layer = "data")
...
counts$Time <- factor(counts$Time, levels = time_order)

ggplot(counts, aes(x = Time, y = n_genes)) +
