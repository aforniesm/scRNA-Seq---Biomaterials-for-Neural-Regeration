...
save.image(file = "~/R/Downloads/AllData_Anchors2.RData")
```

Integration 

```{r}
load(file = "~/R/Downloads/AllData_Anchors2.RData")
gc()
...
save.image(file = "~/R/Downloads/AllData_CCA2.RData")
```

Load integrated dataset

```{r}
load(file = "~/R/Downloads/AllData_CCA2.RData")
gc()
```
...
mapping <- list(
  ...
  "B cells" = c("B cells", "Myeloid, dividing"),
  "Dendritic" = c("Peripheral immune cells", "B cells", "NK, T cells"),
  ...
  "Lymphocyte" = c("Peripheral immune cells", "B cells", "NK, T cells", "Myeloid, dividing"),
  ...
  "NK, T cells" = c("Peripheral immune cells", "NK, T cells", "Myeloid, dividing"),
...
metadata <- seurat_integrated@meta.data
metadata <- metadata %>% select(matches("nCount_RNA|nFeature_RNA|time|celltype|technology|dataset|predicted_celltype"))
...
seurat_integrated@meta.data <- seurat_integrated@meta.data %>%
  mutate(celltype_grouped = case_when(
    ...
    celltype %in% "Newly formed oligodendrocytes (NFOL)" ~ "Newly formed oligodendrocytes",
    ...
    celltype %in% c("B cells", "NK, T cells", "Peripheral immune cells") ~ "Immune cells",
     celltype %in% "Myeloid, dividing" ~ "Myeloid",
...
table(seurat_integrated@meta.data$celltype, seurat_integrated@meta.data$dataset)
...
```{r}
table(seurat_integrated$celltype, seurat_integrated$dataset)
```
